...
bool session_document_write_file(const SessionDocument* doc, const char* path) {
    ...
    json_write_indent(file, 2);
    fprintf(file, "\"low_cut_freq\": ");
    json_write_float(file, doc->effects_panel.low_cut.freq_hz);
    fprintf(file, ",\n");
    json_write_indent(file, 2);
    fprintf(file, "\"low_cut_slope\": ");
    json_write_float(file, doc->effects_panel.low_cut.slope);
    fprintf(file, ",\n");
    json_write_indent(file, 2);
    fprintf(file, "\"high_cut_enabled\": %s,\n", doc->effects_panel.high_cut.enabled ? "true" : "false");
    json_write_indent(file, 2);
    fprintf(file, "\"high_cut_freq\": ");
    json_write_float(file, doc->effects_panel.high_cut.freq_hz);
    fprintf(file, ",\n");
    json_write_indent(file, 2);
    fprintf(file, "\"high_cut_slope\": ");
    json_write_float(file, doc->effects_panel.high_cut.slope);
    fprintf(file, ",\n");
    for (int i = 0; i < 4; ++i) {
        json_write_indent(file, 2);
        fprintf(file, "\"band_%d_enabled\": %s,\n", i, doc->effects_panel.bands[i].enabled ? "true" : "false");
        json_write_indent(file, 2);
        fprintf(file, "\"band_%d_freq\": ", i);
        json_write_float(file, doc->effects_panel.bands[i].freq_hz);
        fprintf(file, ",\n");
        json_write_indent(file, 2);
        fprintf(file, "\"band_%d_gain\": ", i);
        json_write_float(file, doc->effects_panel.bands[i].gain_db);
        fprintf(file, ",\n");
        json_write_indent(file, 2);
        fprintf(file, "\"band_%d_q\": ", i);
        json_write_float(file, doc->effects_panel.bands[i].q_width);
        fprintf(file, "%s\n", i == 3 ? "" : ",");
    }
    json_write_indent(file, 1);
    fprintf(file, "},\n");
    ...
    for (int t = 0; t < doc->track_count; ++t) {
        ...
        json_write_float(file, track->eq.low_cut.freq_hz);
        fprintf(file, ",\n");
        json_write_indent(file, 3);
        fprintf(file, "\"eq_low_cut_slope\": ");
        json_write_float(file, track->eq.low_cut.slope);
        fprintf(file, ",\n");
        json_write_indent(file, 3);
        fprintf(file, "\"eq_high_cut_enabled\": %s,\n", track->eq.high_cut.enabled ? "true" : "false");
        json_write_indent(file, 3);
        fprintf(file, "\"eq_high_cut_freq\": ");
        json_write_float(file, track->eq.high_cut.freq_hz);
        fprintf(file, ",\n");
        json_write_indent(file, 3);
        fprintf(file, "\"eq_high_cut_slope\": ");
        json_write_float(file, track->eq.high_cut.slope);
        fprintf(file, ",\n");
        for (int i = 0; i < 4; ++i) {
            json_write_indent(file, 3);
            fprintf(file, "\"eq_band_%d_enabled\": %s,\n", i, track->eq.bands[i].enabled ? "true" : "false");
            json_write_indent(file, 3);
            fprintf(file, "\"eq_band_%d_freq\": ", i);
            json_write_float(file, track->eq.bands[i].freq_hz);
            fprintf(file, ",\n");
            json_write_indent(file, 3);
            fprintf(file, "\"eq_band_%d_gain\": ", i);
            json_write_float(file, track->eq.bands[i].gain_db);
            fprintf(file, ",\n");
            json_write_indent(file, 3);
            fprintf(file, "\"eq_band_%d_q\": ", i);
            json_write_float(file, track->eq.bands[i].q_width);
            fprintf(file, ",\n");
        }
        json_write_indent(file, 3);
        fprintf(file, "\"clips\": [\n");
