...
- `timeline_view.c`
  - `timeline_view_render`: Paint lane-specific grids, headers, clips, selection handles, playhead, track-aware drag/drop ghosts, and add/remove track buttons.
- `transport.c`
  - `transport_ui_init`: Initialise button/slider rectangles and flags.
  - `transport_ui_layout`: Position transport controls inside the panel.
  - `transport_ui_update_hover/transport_ui_sync`: Update hover state and align slider handles with timeline settings.
  - `transport_ui_render`: Draw buttons, sliders, and labels with hover/active feedback.
