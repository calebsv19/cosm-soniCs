...
static const char* kTestPathA = "assets/audio/test.wav";
static const char* kTestPathB = "assets/audio/sample-9s.wav";
static const char* kTestPathC = "assets/audio/sample-12s.wav";
...
static void exercise_duplicate_acquire(AudioMediaCache* cache) {
    ...
    for (int i = 0; i < kDuplicates; ++i) {
        expect(audio_media_cache_acquire(cache, kTestPathA, 48000, &clips[i]),
...
static void exercise_sample_rate_variants(AudioMediaCache* cache) {
    ...
    expect(audio_media_cache_acquire(cache, kTestPathA, 48000, &clip_48k), "acquire 48k failed");
    expect(audio_media_cache_acquire(cache, kTestPathA, 44100, &clip_44k), "acquire 44.1k failed");
...
static void exercise_capacity_growth(AudioMediaCache* cache) {
    ...
    expect(audio_media_cache_acquire(cache, kTestPathA, 48000, &clip_a), "acquire clip A failed");
    expect(audio_media_cache_acquire(cache, kTestPathB, 48000, &clip_b), "acquire clip B failed");
    expect(audio_media_cache_acquire(cache, kTestPathC, 48000, &clip_c), "acquire clip C failed");
...
static void exercise_churn(AudioMediaCache* cache) {
    ...
    for (int i = 0; i < kCycles; ++i) {
        ...
        expect(audio_media_cache_acquire(cache, kTestPathB, 48000, &clip), "churn acquire failed");
